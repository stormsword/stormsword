...
public class StompEffectScript : MonoBehaviour {
		...
		if(defenderHealth != null) {
			...
			if((ownerType == "Player" && defender.tag == "Enemy")
			   ...
			   (ownerType == "Enemy" && defender.tag == "Player")){
				...
				Snare (defender, snareAmount, snareDuration);
	 ...
	 */
	void Snare(GameObject defender, float snareAmount, float snareDuration) {
//		MoveScript defenderMoveScript = defender.GetComponent<MoveScript>();
		Debug.Log(defender.name + " is snared.");
