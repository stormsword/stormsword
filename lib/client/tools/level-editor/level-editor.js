...
export class LevelEditor extends Phaser.State {
  ...
  placeTile() {
    let tile = this.map.putTile(this.tileSelector.currentTile, this.currentLayer.getTileX(this.marker.x), this.currentLayer.getTileY(this.marker.y), this.layer);

    // Tiles don't use the same angle scale as sprites ;()
    switch(this.tileSelector.cursorTile.angle) {
        case 0:
          break;
        case -90:
          tile.rotation = 11
    ...
    // tile.rotation = Math.abs(this.tileSelector.cursorTile.angle) / 8.181818 // I have no idea why this almost works too
