...
export class Character extends Phaser.Sprite {
  ...
  update() {
    ...
    if(this.chatting) {
      ...
      if(!this.bmd) {
        this.bmd = this.game.make.bitmapData(300, 75)
        this.bmd.rect(0, -100, 64, 120, 'rgba(255,0,255,0.8)')
        this.chatSprite = this.game.add.sprite(0, 0, this.bmd);
      } else {
        this.chatSprite.x = this.x
        this.chatSprite.y = this.y
      }
    } else {
      if(this.bmd) {
        this.bmd = null
