...
export class Character extends Phaser.GameObjects.Sprite {
  ...
  preUpdate() {
    ...
    // Player is walking
    if(this.data) {
      console.log(this.scene.physics.UP)
      // console.log(this.body.facing + " " + Phaser.DOWN)
      switch(this.body.facing) {
        case Phaser.UP: {
          this.play(`${this.name}:${anim}:up`)
          break
        }
        case Phaser.DOWN: {
          this.play(`${this.name}:${anim}:down`)
          break
        }
        case Phaser.LEFT: {
          this.play(`${this.name}:${anim}:left`)
          break
        }
        case Phaser.RIGHT: {
          this.play(`${this.name}:${anim}:right`)
          break
        }
      }
    }
  ...
  loadData(data) {
    ...
    for(let anim in data.anims) {
      this.scene.anims.create({
        key: anim,
        frames: this.scene.anims.generateFrameNumbers(this.name, { frames: data.anims[anim].frames }),
